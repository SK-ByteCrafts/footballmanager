...
while True:
    ...
    for i, contour in enumerate(red_contours):
        ...
        if len(prev_positions['red']) > i:
            if prev_positions['red'][i] is not None and frame_counter % sampling_interval == 0:
                dt = current_time - prev_time
                dx = center[0] - prev_positions['red'][i][0]
                dy = center[1] - prev_positions['red'][i][1]
                velocity = (dx / dt, dy / dt)
                if len(velocities['red']) <= i:
                    velocities['red'].append(deque(maxlen=10))
                velocities['red'][i].append(velocity)
                
                if len(velocities['red'][i]) > 1:
                    dvx = velocities['red'][i][-1][0] - velocities['red'][i][-2][0]
                    dvy = velocities['red'][i][-1][1] - velocities['red'][i][-2][1]
                    acceleration = (dvx / dt, dvy / dt)
                    if len(accelerations['red']) <= i:
                        accelerations['red'].append(deque(maxlen=10))
                    accelerations['red'][i].append(acceleration)
                    avg_acceleration = calculate_average_acceleration(accelerations['red'][i])
                    latest_acceleration['red'].append(avg_acceleration)
            prev_positions['red'][i] = center
        else:
            prev_positions['red'].append(center)
            velocities['red'].append(deque(maxlen=10))
            accelerations['red'].append(deque(maxlen=10))

    # 绘制白色物体检测框，并计算速度和加速度
    latest_acceleration['white'] = []
    for i, contour in enumerate(white_contours):
        ...
        if len(prev_positions['white']) > i:
            if prev_positions['white'][i] is not None and frame_counter % sampling_interval == 0:
                dt = current_time - prev_time
                dx = center[0] - prev_positions['white'][i][0]
                dy = center[1] - prev_positions['white'][i][1]
                velocity = (dx / dt, dy / dt)
                if len(velocities['white']) <= i:
                    velocities['white'].append(deque(maxlen=10))
                velocities['white'][i].append(velocity)
                
                if len(velocities['white'][i]) > 1:
                    dvx = velocities['white'][i][-1][0] - velocities['white'][i][-2][0]
                    dvy = velocities['white'][i][-1][1] - velocities['white'][i][-2][1]
                    acceleration = (dvx / dt, dvy / dt)
                    if len(accelerations['white']) <= i:
                        accelerations['white'].append(deque(maxlen=10))
                    accelerations['white'][i].append(acceleration)
                    avg_acceleration = calculate_average_acceleration(accelerations['white'][i])
                    latest_acceleration['white'].append(avg_acceleration)
            prev_positions['white'][i] = center
        else:
            prev_positions['white'].append(center)
            velocities['white'].append(deque(maxlen=10))
            accelerations['white'].append(deque(maxlen=10))
